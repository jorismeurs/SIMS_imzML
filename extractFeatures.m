classdef extractFeatures < readimzML
    ...
    methods
       ...
       function obj = getUniqueFeatures(obj)
            ...
            obj.uniqueFeatures = tempPeaks;
       end
        
       function obj = alignFeatures(obj)
           tolerance = 0.001;
           featureMatrix = zeros(length(obj.uniqueFeatures),obj.pixelRows*obj.pixelColumns);
            for k = 1:length(obj.uniqueFeatures)
                index = cellfun(@(x) find(x(:,1) > obj.uniqueFeatures(k,1)-tolerance & x(:,1) < obj.uniqueFeatures(k,1)+tolerance),...
                ...
                for p = 1:length(emptyIndex)
                    if emptyIndex(p,1) == true
                        ...
                        if numel(tempIDX) > 1
                            ...
                            difference = obj.uniqueFeatures(k,1)-tempMZ;
